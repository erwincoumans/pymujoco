...
static const int CLIPBOARD_SIZE = 50000;

// copy qpos to clipboard as key
void copykey(void)
{
    char clipboard[CLIPBOARD_SIZE] = "<key qpos='";
    char buf[200];
    int curLen = strlen(clipboard);
    // prepare string
    for( int i=0; i<m->nq; i++ )
    {
        ...
        int l = strlen(buf);
        curLen+=l;
        if ((curLen)<CLIPBOARD_SIZE)
        {
            strcat(clipboard, buf);
        } else
        {
            printf("CLIPBOARD_SIZE too small %d versus %d\n", CLIPBOARD_SIZE, l+curLen );
        }
    }
    strcat(clipboard, "'/>");
